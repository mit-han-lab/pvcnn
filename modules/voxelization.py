...
class Voxelization(nn.Module):
    ...
    def forward(self, features, coords):
        ...
        if self.normalize:
            norm_coords = norm_coords / (norm_coords.norm(dim=1, keepdim=True).max(dim=2, keepdim=True).values
                                         * 2.0) + 0.5
        else:
    ...
    def extra_repr(self):
        return 'resolution={}{}'.format(self.r, ', normalized' if self.normalize else '')
